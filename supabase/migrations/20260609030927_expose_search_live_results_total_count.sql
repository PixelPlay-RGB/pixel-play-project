drop function if exists public.search_live_results(text, integer, text, integer);

create or replace function public.search_live_results(p_query text, p_limit integer default 6, p_section text default null::text, p_offset integer default 0)
 returns table(section text, creator_id uuid, creator_nickname text, creator_photo_url text, broadcast_id uuid, title text, tags text[], thumbnail_url text, current_viewer_count integer, started_at timestamp with time zone, is_live boolean, follower_count integer, is_following boolean, has_more boolean, total_count integer)
 language plpgsql
...
begin
  ...
  if p_section is null or p_section = 'broadcast' then
    ...
    select
      ...
      matched_broadcasts.total_count::integer
  ...
  if p_section is null or p_section = 'creator' then
    ...
    select
      ...
      ranked_creators.total_count::integer
...
grant execute on function public.search_live_results(text, integer, text, integer) to anon, authenticated, service_role;
