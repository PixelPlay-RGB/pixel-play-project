-- 채팅방 목록 RPC의 기본 정렬을 최신 메시지순으로 변경한다.
-- NOT_JOINED 탭은 기존 정책대로 최신 메시지순을 생성일 최신순으로 보정한다.

create or replace function public.get_chat_room_list(
  p_tab_type text,
  p_sort_option text default 'LAST_MESSAGE_DESC',
...
declare
  ...
  v_sort_option text := upper(coalesce(p_sort_option, 'LAST_MESSAGE_DESC'));
...
begin
  ...
  if v_sort_option not in ('CREATED_AT_DESC', 'LAST_MESSAGE_DESC', 'CURRENT_MEMBER_DESC') then
    v_sort_option := 'LAST_MESSAGE_DESC';
  end if;
