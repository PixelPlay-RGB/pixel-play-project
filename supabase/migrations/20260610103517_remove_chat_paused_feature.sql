-- 채팅 일시정지(chat_paused) 기능 제거.
-- 방송 운영 화면에서 정지 버튼이 삭제되어 어디서도 사용하지 않는 기능을 DB에서 걷어낸다.
-- create or replace는 기존 함수의 ACL(service_role 전용)을 보존한다.

-- 1) 전용 쓰기 RPC 제거
drop function if exists public.set_live_chat_paused(uuid, boolean);

-- 2) get_live_watch: settings.chatPaused 노출과 chat_paused 차단 분기 제거
...
-- 3) get_creator_studio_snapshot: settings.chatPaused 노출 제거
...
-- 4) send_live_message_v2: 채팅 일시정지 차단 가드 제거
...
-- 5) 컬럼 제거
alter table public.creator_studio_setting drop column if exists chat_paused;
