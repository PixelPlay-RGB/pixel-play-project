-- Create ENUM types if they don't exist (for CI environment)
DO $$
BEGIN
...
-- Create user table if it doesn't exist (for CI environment)
CREATE TABLE IF NOT EXISTS public."user" (
    ...
    linked_providers public.oauth_provider[] DEFAULT '{}',
    created_at timestamptz DEFAULT now() NOT NULL,
    modified_at timestamptz DEFAULT now() NOT NULL
);

-- Ensure nickname unique constraint exists
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'user_nickname_unique'
    ) THEN
        ALTER TABLE public."user" ADD CONSTRAINT user_nickname_unique UNIQUE (nickname);
    END IF;
END $$;
