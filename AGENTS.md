...
- **자율적 수정 금지**: 사용자의 명시적인 지시(Directive)가 없는 경우, 코드를 직접 수정하거나 새로운 구현을 진행하지 않습니다. 제안이나 분석은 제공할 수 있으나, 실제 파일 수정은 반드시 사용자의 확인을 거친 후 명확한 지시가 있을 때만 수행합니다.
- **단일 책임 원칙 (SRP)**: 모든 컴포넌트, 함수, 클래스는 하나의 역할만 수행해야 합니다. UI 컴포넌트에서 비즈니스 로직을 분리(Hooks 사용)하고, 데이터 처리는 Server Actions에서 담당하도록 설계합니다.
