...
import { BarChart3, Gift, Plus, RotateCw, Sparkles, Trophy, Users, Vote, X } from "lucide-react";
...
export default function ChannelLivePollPanel({ messages }: Props) {
  const [selectedTool, setSelectedTool] = useState<InteractionTool>("poll");
  ...
  return (
    <section className="border-border bg-muted/40 flex min-w-0 flex-col gap-4 rounded-xl border p-4 shadow-sm">
      <div className="grid grid-cols-3 gap-2">
        {INTERACTION_TOOLS.map(({ icon: Icon, label, value }) => (
          <button
            ...
            type="button"
            className={cn(
              "border-border bg-background text-muted-foreground flex min-h-10 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-bold transition-colors",
              "hover:border-brand/40 hover:text-brand",
              selectedTool === value && "border-brand/40 bg-brand/10 text-brand",
            )}
