-- Add aggregate stats to the user donation wallet snapshot.
...
declare
  v_result jsonb;
  v_month_start timestamp with time zone := date_trunc(
    'month',
    ...
    select
      ...
      coalesce(
        sum(donation.amount) filter (
          where donation.created_at >= v_month_start
            and donation.created_at < v_next_month_start
        ),
        0
    ...
    select
      ...
      coalesce(
        sum(wallet_charge.amount_delta) filter (
          where wallet_charge.created_at >= v_month_start
            and wallet_charge.created_at < v_next_month_start
        ),
        0
      )::integer as current_month_amount
