-- 라이브 채팅 발신자 역할(sender_role)을 메시지 전송 시점 스냅샷으로 기록한다.
-- 마크 표시가 클라이언트 누적(세션 중 본 후원 메시지)에 의존하지 않게 하고,
-- manager·subscriber는 추후 기능(매니저 지정·구독)을 대비해 enum에 미리 둔다.

create type public.live_sender_role as enum ('creator', 'manager', 'donor', 'subscriber', 'viewer');

alter table public.live_message
  add column sender_role public.live_sender_role not null default 'viewer';

-- 백필: 방장 메시지 → creator, 전송 시점 후원 이력이 기록된 메시지 → donor.
update public.live_message as message
set sender_role = 'creator'
from public.live_broadcast as broadcast
where message.broadcast_id = broadcast.id
  and message.sender_id = broadcast.creator_id
  and message.message_type = 'chat'::public.live_message_type;

update public.live_message
set sender_role = 'donor'
where message_type = 'chat'::public.live_message_type
  and sender_role = 'viewer'
  and (metadata ->> 'isDonor') = 'true';

-- send_live_message_v2: INSERT에 sender_role 스냅샷을 추가한다(나머지 로직 동일).
...
begin
  ...
  v_sender_role := case
    when p_actor_user_id = v_broadcast.creator_id then 'creator'::public.live_sender_role
