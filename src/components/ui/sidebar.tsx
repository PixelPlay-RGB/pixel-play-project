...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "no-scrollbar flex min-h-0 flex-1 flex-col gap-0 overflow-auto py-3 group-data-[collapsible=icon]:overflow-hidden",
        className,
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  // Math.random은 서버와 클라이언트가 서로 다른 값을 만들어 hydration mismatch를 일으키므로,
  // 양쪽에서 동일한 useId를 해시해 50~90% 범위의 결정적 폭을 만든다.
  ...
  return (
    <div
      ...
      className={cn("flex h-8 items-center gap-5 rounded-md px-2", className)}
      {...props}
    >
