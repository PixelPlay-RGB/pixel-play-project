...
export function TutorialDialog({ title, steps, trigger, triggerLabel = "가이드 보기" }: Props) {
  ...
  return (
    ...
    >
      ...
      />
      {/* 높이를 90vh로 고정해 단계마다 설명 길이가 달라도 이전/다음 버튼 위치가 움직이지 않는다. */}
      <DialogContent className="flex h-[90vh] flex-col sm:max-w-[92vw] 2xl:max-w-[1400px]">
        ...
        <button
          ...
          // 이미지가 남는 세로 공간을 모두 흡수한다(flex-1) — 디테일은 클릭 확대 보기로 확인한다.
          className="border-border bg-muted/40 hover:border-brand/60 group relative min-h-0 w-full flex-1 overflow-hidden rounded-xl border transition-colors duration-150"
        >
          <Image
        ...
        {/* 설명 영역은 5줄 분량으로 넉넉히 고정해 스크롤 없이 전부 보이게 한다(버튼 행 위치 고정 겸).
            남는 세로 공간은 이미지(flex-1)가 흡수하며 디테일은 클릭 확대로 본다 — 설명이
            5줄(현재 최장 4줄)을 넘게 되면 이 높이도 함께 늘린다. */}
        <HintNote className="h-38 shrink-0 text-sm leading-6 whitespace-pre-line">
