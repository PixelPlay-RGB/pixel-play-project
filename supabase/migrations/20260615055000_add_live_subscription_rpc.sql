-- 라이브 구독 RPC와 구독자 채팅 뱃지 판정을 추가한다.
...
begin
  ...
  select subscription.status = 'active'::public.creator_subscription_status
  ...
  v_was_active := coalesce(v_was_active, false);
        ...
        'viewerRelation', case
          when p_viewer_id is null then 'null'::jsonb
            ...
            'isSubscribed', coalesce(
              subscription.status = 'active'::public.creator_subscription_status
...
begin
  ...
  select exists (
    ...
    where subscription.subscriber_id = p_actor_user_id
      and subscription.creator_id = v_broadcast.creator_id
