-- Extend /live list RPC into a single JSON snapshot contract.
-- Recent chat filtering and sorting use chat messages from the last 5 minutes.

drop function if exists public.get_live_list(text, integer, integer);

create index if not exists live_message_recent_chat_broadcast_created_at_idx
  on public.live_message using btree (broadcast_id, created_at desc)
  where message_type = 'chat'::public.live_message_type;
...
begin
  ...
  filtered_broadcast as (
    ...
    left join public.viewer_creator_relation as relation
      on relation.viewer_id = p_viewer_id
    ...
    where broadcast.ended_at is null
      ...
      and (
        v_filter <> 'FOLLOWING'
        or (p_viewer_id is not null and relation.viewer_id is not null)
      )
      and (
        v_filter <> 'RECENT'
...
revoke execute on function public.get_live_list(text, text, uuid, text, integer, integer) from public;
grant execute on function public.get_live_list(text, text, uuid, text, integer, integer) to anon;
grant execute on function public.get_live_list(text, text, uuid, text, integer, integer) to authenticated;
grant execute on function public.get_live_list(text, text, uuid, text, integer, integer) to service_role;
