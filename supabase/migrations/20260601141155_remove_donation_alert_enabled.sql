-- '화면 알림(donation_alert_enabled)' 토글 제거: 전용 오버레이라 URL 등록 여부로 충분.
-- 1) studio 스냅샷에서 donationAlertEnabled 제거
...
-- 2) 후원 알림 오버레이 RPC에서 게이팅/플래그 제거 (URL 등록 여부로만 제어)
...
-- 3) upsert에서 p_donation_alert_enabled 파라미터·컬럼 참조 제거 (시그니처 변경)
...
REVOKE EXECUTE ON FUNCTION public.upsert_creator_studio_setting(
  uuid, text, text[], live_chat_scope, integer, boolean, integer, boolean, text[], text,
  boolean, integer, boolean, integer, boolean, integer, boolean, numeric, jsonb,
  boolean, text, integer, text
) FROM PUBLIC, anon, authenticated;

-- 4) 컬럼 제거
ALTER TABLE public.creator_studio_setting DROP COLUMN IF EXISTS donation_alert_enabled;
