"use client";
// 구독 퍼스나콘 개월 수와 이미지를 등록하는 Dialog를 렌더링합니다.
...
export function PersonaconRegistrationDialog({ open, onOpenChange }: Props) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        console.error("구독 퍼스나콘 등록 실패", error);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-y-auto p-0 sm:max-w-md">
        <DialogHeader className="px-7 pt-8 pb-4">
          <DialogTitle className="text-xl font-black">구독 퍼스나콘 등록</DialogTitle>
          <DialogDescription className="sr-only">
            구독 개월 수와 퍼스나콘 이미지를 등록합니다.
          </DialogDescription>
        </DialogHeader>

        <form className="flex flex-col gap-5 px-7 pb-8" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2">
            <label htmlFor="personacon-month" className="text-sm font-black">
              퍼스나콘 개월 수
            </label>
          ...
          <div className="flex flex-col gap-2">
            ...
            <ul className="text-muted-foreground list-disc space-y-1 pl-5 text-xs leading-5">
              <li>
                사이즈: {CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE}x
                {CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE} / 파일크기:{" "}
            ...
            <LargeUploadButton
              label="이미지 업로드"
              detail={`(${CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE}x${CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE})`}
          ...
          <ul className="text-muted-foreground list-disc space-y-2 pl-5 text-xs leading-5">
            <li>신청한 퍼스나콘은 기본적인 검수 진행 후 1일 1회 일괄 등록 예정입니다.</li>
            <li>기본 퍼스나콘 슬롯은 1개월과 동일하며 이미지 등록 및 수정, 삭제가 가능합니다.</li>
            <li>개월 수는 직접 선택 및 입력이 가능합니다.</li>
          </ul>

          <InfoNotice>
            {CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE}x{CHANNEL_SUBSCRIPTION_BADGE_IMAGE_SIZE}
            사이즈로 등록해주시면 이미지가 선명해요.
          </InfoNotice>
